...
class ProductClassifier:
    """Классификатор продуктов с использованием Ollama"""
    ...
    def load_model(self) -> bool:
        ...
        try:
            logger.info(f"Загружаем модель {self.model_name} через Ollama...")
            ...
            if result.returncode != 0:
                logger.error("Ollama не запущен или не установлен")
                return False
            
            if self.model_name not in result.stdout:
                logger.error(f"Модель {self.model_name} не найдена")
                return False
